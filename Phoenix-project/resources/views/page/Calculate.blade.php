...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        roomNum.addEventListener("input", function() {
            if (roomNum.value.trim() !== "") {
                roomContainer.innerHTML = ""; // Clear previous RoomS inputs
    ...
    bodojOloh.addEventListener("click", function() {
            ...
            roomSInputs.forEach((input, index) => {
                ...
                // hamgiin tom halaaguuraas ehleh 
                heaters.forEach(heater => {
                    ...
                    if (numHeaters > 0) {
                        ...
                        // tootsolol hiih tomyo
                        roomBill += numHeaters * heater.kW * 147 * 15 * 30 * 0.6;
                    }
                });

                // halaaguur uldwel ahij halaaguur nemeh
                if (remainingSize > 0) {
                    roomHeaters.push({ heaterName: heaters[heaters.length - 1].name, count: 1 });
                    // Calculate the bill for the extra heater
                    roomBill += heaters[heaters.length - 1].kW * 147 * 15 * 30 * 0.6;
                }

                // niit tsahilgaani zardal 
                totalBill += roomBill;

                // halaaguur card nemeh
